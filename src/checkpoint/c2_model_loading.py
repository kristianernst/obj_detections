...
def convert_basic_c2_names(original_keys):
  ...
  layer_keys = [{"pred_b": "linear_b", "pred_w": "linear_w"}.get(k, k) for k in layer_keys]  # some hard-coded mappings
...
def convert_c2_detectron_names(weights):
  ...
  for orig, renamed in zip(original_keys, layer_keys):
    ...
    if renamed.startswith("bbox_pred.") or renamed.startswith("mask_head.predictor."):
      ...
      logger.info(
        "Remove prediction weight for background class in {}. The shape changes from " "{} to {}.".format(
      ...
      logger.info(
        "Move classification weights for background class in {} from index 0 to " "index {}.".format(renamed, weights[orig].shape[0] - 1)
      )
...
def align_and_update_state_dicts(model_state_dict, ckpt_state_dict, c2_conversion=True):
  ...
  for idx_model, idx_ckpt in enumerate(idxs.tolist()):
    ...
    if shape_in_model != value_ckpt.shape:
      logger.warning(
        "Shape of {} in checkpoint is {}, while shape of {} in model is {}.".format(key_ckpt, value_ckpt.shape, key_model, shape_in_model)
      )
    ...
    if key_ckpt in matched_keys:  # already added to matched_keys
      logger.error(
        "Ambiguity found for {} in checkpoint!" "It matches at least two keys in the model ({} and {}).".format(
