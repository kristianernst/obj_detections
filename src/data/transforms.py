  ...
  def __repr__(self):
    ...
    try:
      ...
      for name, param in sig.parameters.items():
        ...
        assert hasattr(self, name), "Attribute {} not found! " "Default __repr__ only works if attributes match the constructor.".format(
          name
        )
...
class ResizeShortestEdge(Augmentation):
  ...
  def __init__(self, short_edge_length, max_size=sys.maxsize, sample_style="range", interp=Image.BILINEAR):
    ...
    if self.is_range:
      assert len(short_edge_length) == 2, "short_edge_length must be two values using 'range' sample style." f" Got {short_edge_length}!"
    self._init(locals())
