...
from models.layers import (Conv2d, batched_nms, cat, move_device_like,
                           subsample_labels)
...
class RPN(nn.Module):
  ...
  def label_and_sample_anchors(self, anchors: List[Boxes], gt_instances: List[Instances]) -> Tuple[List[torch.Tensor], List[torch.Tensor]]:
    ...
    for image_size_i, gt_boxes_i in zip(image_sizes, gt_boxes):
      ...
      match_quality_matrix = retry_if_gpu_oom(pairwise_iou)(gt_boxes_i, anchors)
      matched_idxs, gt_labels_i = retry_if_gpu_oom(self.anchor_matcher)(match_quality_matrix)
