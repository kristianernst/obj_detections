...
from models.head.generators.box_regression import _dense_box_regression_loss
from models.head.structures import Boxes, Instances
from models.layers import (ShapeSpec, batched_nms, cat, cross_entropy,
                           nonzero_tuple)
...
def fast_rcnn_inference(
  ...
  """
  result_per_image = [
    fast_rcnn_inference_single_image(boxes_per_image, scores_per_image, image_shape, score_thresh, nms_thresh, topk_per_image)
...
class FastRCNNOutputLayers(nn.Module):
  ...
  def predict_boxes_for_gt_classes(self, predictions, proposals):
    ...
    if K > 1:
      ...
      predict_boxes = predict_boxes.view(N, K, B)[torch.arange(N, dtype=torch.long, device=predict_boxes.device), gt_classes]
