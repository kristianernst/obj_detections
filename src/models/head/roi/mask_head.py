...
from models.head.structures import Instances
from models.layers import (Conv2d, ConvTranspose2d, ShapeSpec, cat, get_norm,
                           move_device_like)
from util.events import get_event_storage
from util.load_weights import c2_msra_fill
...
def mask_rcnn_loss(pred_mask_logits: torch.Tensor, instances: List[Instances], vis_period: int = 0):
  ...
  for instances_per_image in instances:
    ...
    gt_masks_per_image = instances_per_image.gt_masks.crop_and_resize(instances_per_image.proposal_boxes.tensor, mask_side_len).to(
...
def mask_rcnn_inference(pred_mask_logits: torch.Tensor, pred_instances: List[Instances]):
  ...
  if cls_agnostic_mask:
    ...
    device = class_pred.device if torch.jit.is_scripting() else ("cpu" if torch.jit.is_tracing() else class_pred.device)
