...
from models.head.generators.proposal_generator import \
    add_ground_truth_to_proposals
from models.head.roi.fast_rcnn import fast_rcnn_inference
from models.head.structures import Boxes, ImageList, Instances, pairwise_iou
from models.layers import nonzero_tuple, subsample_labels
...
def select_foreground_proposals(proposals: List[Instances], bg_label: int) -> Tuple[List[Instances], List[torch.Tensor]]:
    """
...
class CascadeROIHeads(StandardROIHeads):
    ...
    def _forward_box(self, features, proposals, targets=None):
        ...
        if self.training:
            ...
            scores = [
                sum(list(scores_per_image)) * (1.0 / self.num_cascade_stages) for scores_per_image in zip(*scores_per_stage)
            ]
