...
class PolygonMasks:
  ...
  def __init__(self, polygons: List[List[Union[torch.Tensor, np.ndarray]]]):
    ...
    if not isinstance(polygons, list):
      raise ValueError(
        "Cannot create PolygonMasks: Expect a list of list of polygons per image. " "Got '{}' instead.".format(type(polygons))
      )
    ...
    def process_polygons(polygons_per_instance: List[Union[torch.Tensor, np.ndarray]]) -> List[np.ndarray]:
      if not isinstance(polygons_per_instance, list):
        raise ValueError(
          "Cannot create polygons: Expect a list of polygons per instance. " "Got '{}' instead.".format(type(polygons_per_instance))
        )
...
class ROIMasks:
  ...
  def to_bitmasks(self, boxes: torch.Tensor, height, width, threshold=0.5):
    ...
    """
    from models.layers import _paste_masks_tensor_shape, paste_masks_in_image
