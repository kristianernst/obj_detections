...
from models.head.structures import (BitMasks, Boxes, BoxMode, Keypoints,
                                    PolygonMasks, RotatedBoxes)
from util.colors import _COLORS
from util.logger import log_first_n
...
def make_dist_plot(weightlayer: torch.Tensor, name: str, outdir: str):
  """Make a distribution plot of the weights"""

  if not os.path.exists(outdir):
    os.makedirs(outdir, exist_ok=True)
    plt.figure(figsize=(10, 6))
  plt.hist(weightlayer.data.flatten().cpu().numpy(), bins=50, edgecolor="black")
  plt.title(f"Weight Distribution: {name}")
  plt.xlabel("Weight Value")
  plt.ylabel("Frequency")
  plt.grid(True, alpha=0.3)
  plt.tight_layout()
  plt.savefig(f"{outdir}/{name}_distribution.png", dpi=300)
  plt.close()
  ...
  def __getattr__(self, key):
    ...
    if len(self.__dict__) > 1:
      raise AttributeError(
        "Attribute '{}' does not exist in the metadata of dataset '{}'. Available " "keys are {}.".format(
